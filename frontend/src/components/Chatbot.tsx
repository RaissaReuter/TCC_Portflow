          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-gray-50">
            {messages.map((msg, index) => (
              <div key={index} className={`flex items-start gap-2.5 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                {msg.sender === 'assistant' && (
                  <Image src="/images/logoPortinho.png" alt="Portinho" width={50} height={50} className="w-8 h-8 rounded-full" />
                )}
            ...
            {isLoading && (
              <div className="flex items-start gap-2.5">
                <Image src="/images/logoPortinho.png" alt="Portinho" width={60} height={60} className="w-8 h-8 rounded-full" />
