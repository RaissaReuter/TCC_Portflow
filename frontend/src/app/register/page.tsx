// ARQUIVO CORRIGIDO: frontend/src/app/register/page.tsx
"use client";
...
export default function RegisterPage() {
  ...
  const searchParams = useSearchParams(); // 2. DECLARAR searchParams
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // No seu código original, você ainda estava enviando 'role: aluno'. Removi isso para seguir a nova lógica.
      await axios.post('http://localhost:3001/api/users/register', { name, email, password });
      toast.success("Cadastro realizado com sucesso! Faça o login.");
      
      // 3. CORRIGIR O REDIRECIONAMENTO
  ...
  const handleGoogleSuccess = async (credentialResponse: CredentialResponse) => {
    ...
    try {
      ...
      // 4. ADICIONAR LÓGICA DE REDIRECIONAMENTO AQUI TAMBÉM
      ...
      <div className="min-h-screen bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
          ...
          <p className="mt-6 text-center text-sm text-gray-600">
            Já tem uma conta?{" "}
            {/* 5. CORRIGIR O LINK PARA PASSAR O PARÂMETRO */}
            <Link 
